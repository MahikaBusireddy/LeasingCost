public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        int i1 = description.indexOf("type:")+"type:".length();
        int i2 = description.indexOf("length:")+ "length:".length();
        int i3 = description.indexOf("due:")+"due:".length();
        int i4 = description.indexOf("name:")+"name:".length();
        int i5 = description.indexOf("monthly:")+"monthly:".length();
        int i6 = description.indexOf("mile/unit:")+"mile/unit:".length();
        int i7 = description.indexOf("allowance:")+"allowance:".length();
        int i8 = description.indexOf("charger:")+"charger:".length();
       
       
        String fueltype = description.substring(i1,description.indexOf(';',i1));
        String name = description.substring(i4,description.indexOf(";",i4));
        String due = description.substring(i3,description.indexOf(';',i3));
        String mileunit = description.substring(i6,description.indexOf(';',i6));
        String monthly = description.substring(i5,description.indexOf(';',i5));
        String allowance = description.substring(i7,description.indexOf(';',i7));
        String length = description.substring(i2,description.indexOf(';',i2));       
        String x = "0.0";

        if(i8 >= 0)
            x = description.substring(i8,description.indexOf(';',i8));
        ...
        if(fueltype.equalsIgnoreCase("gas")) 
            ...
            double charger = Double.parseDouble(x);
            f = new Fuel(usage, charger);
        }
       
        double signDue = Double.parseDouble(due);
        int leaseLength = Integer.parseInt(length);
        double cost = Double.parseDouble(monthly);
        int allowance1 = Integer.parseInt(allowance);
        Lease l = new Lease(signDue, leaseLength, cost, allowance1);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for (Vehicle car: vehicles){
            int num = car.getLease().getLeaseLength();
            double usage = car.getFuel().getUsage();
            double allowance = car.getLease().getMileageAllowance();
            double due = car.getLease().getDueAtSigning();
            double monthly = car.getLease().getMonthlyCost();
            int numberofMonths = car.getLease().getLeaseLength();
            double charge = 0;

            if (car.getFuel().getType() == 1){
                car.setFuelCost(computeFuelCost(num, usage, allowance, gasPrice));
                car.setCO2Emission(computeCO2(num, usage, allowance, Fuel.CO2EMITTED_GASCOMBUSTION));
            }
            else{
                car.setFuelCost(computeFuelCost(num, usage, allowance, electricityPrice));
                car.setCO2Emission(computeCO2(num, usage, allowance, Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION));
                charge = car.getFuel().getCharger();
            }
            car.setTotalCost(car.getFuelCost() + computeLeaseCost(due, numberofMonths, monthly) + charge);
        }
